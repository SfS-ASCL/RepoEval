...
class OAIEval:
    ...
    def validate_oai(self, xml):
        ...
        for cmdi in all_cmdis:
            ...
            if cmdi_page.status_code == 404:
                self.add_error(f"404 -              {cmdi_handle}", cmdi_handle)
    ...
    def _validate_acl(self, cmdi, res_url, res_handle, cmdi_handle):
        ...
        if r is None: 
            self.add_error(f"ACL is None       - {acl_url}", cmdi_handle)
            return
        try:      
            acl_dict = json.loads(r.content)
        except JSONDecodeError:
            print("JSONDecodeError")
            self.add_error(f"ACL can't be parsed - {acl_url}", cmdi_handle)
            return
        ...
        try:
            ...
            if acl in self.acl_restricted:
                ...
                if len(t) == 0:
                    self.add_error(f"ACL incorrect -     {res_handle}", cmdi_handle)
                return
        except:
            return # True
        self.add_error(f"ACL incorrect -     {res_handle}", cmdi_handle)
    ...
    def _validate_checksum(self, resource, res_handle, handle):  
        ...
        for k, v in cmdi_checksums.items():
            ...
            if v is not None and str(calc_v) != str(v) and str(v).strip() != "":
                self.add_error(f"Checksums wrong - {res_handle}: CMDI: {str(v)}, Computed: {str(calc_v)} [{k}]", handle)
                valid = False   
        return valid
